...
namespace UnityAsapIntegration.ASAP.Editor {
	...
	public class BMLTestEditor : EditorWindow {
		...
		private static BMLTestEditor w;

		private UnityAsapIntegration.ASAP.BMLTestAsset testBmlAsset;
		...
		[MenuItem("ASAP/BML Test Editor")]
		static void Init() {
			w = GetWindow<BMLTestEditor>(false, "BML Editor", true);
			w.Show();
			w.Populate();
		}
		...
		void Populate() {
			...
			guids = AssetDatabase.FindAssets("t:BMLTestAsset");
			labels = new string[guids.Length];
			...
			for (int i = 0; i < guids.Length; i++) {
				BMLTestAsset bmlta = AssetDatabase.LoadAssetAtPath<BMLTestAsset>(AssetDatabase.GUIDToAssetPath(guids[i]));
				labels[i] = bmlta.name;
			}
			...
			if (guids.Length > 0) {
				...
				testBmlAsset = AssetDatabase.LoadAssetAtPath<BMLTestAsset>(AssetDatabase.GUIDToAssetPath(guids[selection]));
		...
		private void OnGUI() {
			...
			if (GUILayout.Button("SEND ALL", sndStyle, GUILayout.Height(19.0f))) {
				for (int i = 0; i < testBmlAsset.bmls.Length; i++) {
					FindObjectOfType<BMLRequests>().SendBML(testBmlAsset.bmls[i]);
				}
			}
