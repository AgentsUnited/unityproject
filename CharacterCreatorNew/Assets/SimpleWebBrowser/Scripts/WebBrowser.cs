...
namespace SimpleWebBrowser
{
    ...
    public class WebBrowser : MonoBehaviour
    {
        ...
        void Start()
        {
			...
			apolloReaderThreadURL = new Thread(() => ApolloReader("COUCH/UI/URL"));
			apolloReaderThreadURL.Start ();
		...
		public void OnApplicationQuit() {
			...
			if (apolloReaderThreadURL != null && !apolloReaderThreadURL.Join(500)) { 
				Debug.LogWarning("Could not close apolloReaderThreads");
				apolloReaderThreadURL.Abort();
			}
		...
		void OnSTOMPMessage(IMessage receivedMsg) {
			...
			if (message.NMSDestination.ToString().Equals("topic://COUCH/UI/URL"))
			{
				URL_Control(msg);
			}
